...
class ArrayQueue:
  ...
  def dequeue(self):
    ...
    self.__n -= 1
    self.__c = self.__c - 1 if self.__c > 0 else 0
    return answer

  def get_current(self):
    if self.__n == 0: raise Exception('队列为空')
    current = (self.__h + self.__c) % len(self.__A)
    elem = self.__A[current]
    self.__c = (self.__c + 1) % self.__n
    return elem
